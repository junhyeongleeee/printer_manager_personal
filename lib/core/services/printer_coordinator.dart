...
class PrinterCoordinator {
  ...
  Future<Map<ManagedPrinter, bool>> assignPrintJobsConcurrently({required List<PrintJobAssignment> assignments}) async {
    final results = <ManagedPrinter, bool>{};
    ...
    final futures = assignments.map((assignment) async {
      try {
        ...
        return MapEntry(_printers.firstWhere((p) => p.id == assignment.printerId), false);
      }
    });
  ...
  Future<void> disconnectAll() async {
    final futures = _printers.map((printer) => Future(() => printer.dispose()));
