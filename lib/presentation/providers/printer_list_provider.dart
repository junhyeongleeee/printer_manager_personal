...
StateNotifierProvider<PrinterListNotifier, List<ManagedPrinter>>(
...
class PrinterListNotifier extends StateNotifier<List<ManagedPrinter>> {
  ...
  void assignPrintJob(int printerid, int jobId, String itemName, String code, int start, int end, bool isLastPrint) async {
    _useCase = ref.read(printJobUseCaseProvider);

    print("assignPrintJob Start");
    ...
    String jobName = "a";
    print("printer.printStatus: ${printer.printStatus}");
    final request_start = UpdatePrinterjobRequest(
      status: "IN_PROGRESS",
      message: "인쇄 중",
    );
    ...
    // );
    print("printerId: $printerid, jobId: $jobId, itemName: $itemName, code: $code, start: $start, end: $end");
    ...
    print("updatePrinterJob response: $response");
    ...
    print("updatePrinterJob response: $response2");
  ...
  void mergeNewData(WidgetRef ref, PrinterListResponse newDtos) {
    ...
    final newItems = newDtos.data.printerList
        .where((dto) =>
        dto.status != '삭제' &&
        !currentIds.contains(dto.processingCompanyPrinterIndex))
